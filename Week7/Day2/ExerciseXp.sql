...
-- The first 3 customers in alphabetical order of the first name (A-Z), excluding the primary key column
...
--Write a query to get the film ID, title, description, year of release and rental 
...
--You want to get the first name and last name from the curstomer table, as well as the 
...
--Write a query to get the customer’s id, names (first and last), the amount and the date of payment ordered by the id 
--of the staff member who sold them the dvd.

SELECT p.customer_id, c.first_name, c.last_name, p.amount, p.payment_date FROM payment p
